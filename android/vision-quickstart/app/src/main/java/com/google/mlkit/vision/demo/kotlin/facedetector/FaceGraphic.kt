...
class FaceGraphic constructor(overlay: GraphicOverlay?, private val face: Face) : Graphic(overlay) {
    ...
    override fun draw(canvas: Canvas) {
        ...
        if (face.leftEyeOpenProbability != null) {
            canvas.drawText(
                "Right eye: " + if (face.leftEyeOpenProbability!! > 0.7) "open" else "closed",
                //"Left eye open: " + if(face.leftEyeOpenProbability> 0.7)String.format(Locale.US, "%.2f", face.leftEyeOpenProbability),
                left,
                top + yLabelOffset,
                idPaints[colorID]
            )
        ...
        if (face.rightEyeOpenProbability != null) {
            canvas.drawText(
                "Left eye: " + if (face.rightEyeOpenProbability!! > 0.7) "open" else "closed",
